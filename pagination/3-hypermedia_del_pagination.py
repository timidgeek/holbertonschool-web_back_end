...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        ...
        for _ in range(page_size):
            # increment `next_index` until it's ahead of `indexed_dataset`
            # and then add data to `indexed_dataset`
            while not self.indexed_dataset().get(next_index):
                next_index += 1
            data.append(self.indexed_dataset()[next_index])
            next_index += 1

        # connect all collected data to hyper dict
        return {
            "index": index,
            "data": data,
            "page_size": page_size,
